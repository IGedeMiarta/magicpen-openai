        ...
        <div class="container h-100vh">
            <div class="row text-center h-100vh align-items-center">
                <div class="col-md-12">
                    ...
                    <h5>{{ __('We apologize for the inconvenience but') }} <span
                            class="font-weight-bold text-info">{{ config('app.name') }}</span>
                        {{ __('is currenlty undergoing planned maintenance') }}.</h5>
                </div>
            </div>
        </div>
    @else
            ...
            <div class="container-fluid justify-content-center">
                <div class="row h-100vh align-items-center background-white">
                    <div class="col-md-7 col-sm-12 text-center background-special h-100 align-middle p-0"
                        id="login-background">
                        <div class="login-bg" style="background-image: {{ asset('img/files/login-bg.png') }}"></div>
                    </div>
                    ...
                    <div class="col-md-5 col-sm-12 h-100" id="login-responsive">
                        <div class="card-body pr-10 pl-10 pt-10">
                            <form method="POST" action="{{ route('register') }}">
                                ...
                                <h3 class="text-center font-weight-bold mb-8">{{ __('Sign Up to') }} <span
                                    ...
                                    <div class="actions-total text-center">
                                        @if (config('services.facebook.enable') == 'on')
                                            <a href="{{ url('/auth/redirect/facebook') }}"
                                                data-tippy-content="{{ __('Login with Facebook') }}" class="btn mr-2"
                                ...
                                <div class="input-box mb-4">
                                    ...
                                    <input id="name" type="name"
                                        ...
                                        placeholder="{{ __('First and Last Names') }}">
                                    @error('name')
                                ...
                                <div class="input-box mb-4">
                                    ...
                                    <select id="user-country" name="country"
                                        data-placeholder="{{ __('Select Your Country') }}" required>
                                ...
                                <div class="input-box">
                                    <label class="fs-12 font-weight-bold text-md-right">{{ __('Referrals Code') }}</label>
                                    <input type="text" class="form-control" name="ref" autocomplete="off"
                                        placeholder="{{ __('Referrals Code') }}"
                                        @if (request()->get('ref')) readonly value="{{ request()->get('ref') }}" @endif>
                                </div>
                                <div class="input-box d-none  ">
                                    <label class="fs-12 font-weight-bold text-md-right">{{ __('comunity') }}</label>
                                    <input type="text" class="form-control" name="com" autocomplete="off"
                                        placeholder="{{ __('Comunity id') }}"
                                        @if (request()->get('com')) readonly value="{{ request()->get('com') }}" @endif>
                                </div>
